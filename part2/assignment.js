...
function max(arr) {
  var maxOfArray = Math.max.apply(Math, arr);
  return maxOfArray;
}
...
function min(arr) {
  var minOfArray = Math.min.apply(Math, arr);
  return minOfArray;
}
...
function distance(point1, point2) {
  ...
  var x2 = point2.x;
  var y2 = point2.y;
  var coordinates = Math.sqrt(Math.pow((x2-x1), 2) + Math.pow((y2-y1), 2));
  return coordinates;
}
...
function combine(obj1, obj2) {
  var object3 = {};
  for (var key in obj1) {
    object3[key] = obj1[key];
  }
  for (var key in obj2) {
    object3[key] = obj2[key];
  }
  return object3;
}
